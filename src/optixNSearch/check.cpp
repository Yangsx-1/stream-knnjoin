...
static inline void report_procedure_percentage(unsigned int count, unsigned int sample_length) {
  if ((count & 255) != 0) return;
  std::cout << "\r" << "check percentage: " << 100.0 * count / sample_length << "%" << std::flush;
}
...
void sanityCheckKNN( RTNNState& state, int batch_id ) {
  ...
  std::vector<unsigned int> randQ(state.numQueries);
  std::iota(randQ.begin(), randQ.end(), 0);
  std::random_device rd;
  std::mt19937 gen(rd());
  std::vector<unsigned int> outQ;
  unsigned int sample_length = state.numQueries / 1000;
  outQ.resize(sample_length);
  std::sample(randQ.begin(), randQ.end(), outQ.begin(), sample_length, gen);
  std::unordered_set<unsigned int> checking_querys(outQ.begin(), outQ.end());
  unsigned int count = 0, nok_count = 0, incorrect_count = 0;
  std::cout << "need finding neighbors: " << state.knn << std::endl;
  std::cout << "Checking query length: " << checking_querys.size() << std::endl;
  std::cout << std::setiosflags(std::ios::fixed) << std::setprecision(4);

  for (unsigned int q = 0; q < state.numQueries; q++) {
    if (checking_querys.find(q) == checking_querys.end()) continue;
    count++;
    ...
    if (printRes) std::cout << "found neighbors: " << gpu_idxs.size();
    if (printRes) std::cout << std::endl;
    if (gpu_idxs.size() < state.knn) nok_count++;
    ...
    report_procedure_percentage(count, sample_length);
  }
  std::cout << "\r" << "No k neighbor ratio: " << 100.0 * nok_count / count << "%" << std::endl;
  std::cout << "Incorrect count: " << 100.0 * incorrect_count / count << "%" << std::endl;
